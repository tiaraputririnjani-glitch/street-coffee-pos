...
<head>
    ...
    <title>Street Coffee POS - Master Final Perfection</title>
...
<body class="bg-gray-100 font-sans text-gray-900 scroll-smooth">

    <div id="shift-modal" class="fixed inset-0 bg-black/95 hidden items-center justify-center z-[9999] p-4 backdrop-blur-2xl no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl border-4 border-orange-500">
            ...
            <p class="text-[10px] text-gray-400 font-bold mb-10 italic uppercase">Wajib Isi Sebelum Operasional</p>
            <div class="space-y-5 mb-10">
                <div class="text-left">
                    <label class="text-[10px] font-black text-gray-400 uppercase ml-2 mb-1 block">Nama Kasir Bertugas</label>
                    <input type="text" id="cashier-name-input" placeholder="Masukkan Nama Kasir..." class="w-full p-5 bg-gray-50 border border-gray-100 rounded-3xl outline-none text-center font-black text-gray-700 focus:ring-4 focus:ring-orange-200 text-lg">
                </div>
                <div class="text-left">
    ...
    <div id="close-shift-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-[8000] p-4 backdrop-blur-md no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl relative border-4 border-gray-900">
            ...
            <button onclick="endShift()" class="w-full bg-gray-900 text-white py-5 rounded-3xl font-black uppercase shadow-xl hover:bg-black active:scale-95 transition-all">Kirim Laporan Audit</button>
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row min-h-screen md:h-screen overflow-hidden bg-gray-100">
        ...
        <div class="w-full md:w-20 bg-white shadow-sm md:shadow-lg flex flex-row md:flex-col items-center justify-evenly md:justify-start py-4 md:py-6 space-x-2 md:space-x-0 md:space-y-8 z-30 sticky top-0 md:relative no-print">
            ...
            <div class="md:mt-auto flex flex-row md:flex-col items-center space-x-4 md:space-x-0 md:space-y-4">
                ...
                <form action="{{ route('logout') }}" method="POST">@csrf <button type="submit" class="p-2 md:p-3 text-gray-300 hover:text-red-500 transition-colors cursor-pointer">🚪</button></form>
            </div>
        </div>
        ...
        <div class="flex-1 p-4 md:p-8 overflow-y-auto h-full bg-gray-50/50 custom-scroll">
            ...
            <div class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-xl flex items-center justify-between shadow-sm no-print">
                <div class="flex items-center">
                    ...
                    <p class="text-xs font-black text-red-700 uppercase tracking-tighter">Owner, ada {{ $lowStockCount }} bahan menipis! Segera belanja lgi.</p>
                </div>
            ...
            <div id="section-pos" class="space-y-8 no-print">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p id="active-cashier-label" class="text-gray-400 text-[10px] font-black uppercase tracking-widest italic">Street Coffee POS</p>
                    </div>
                    @if(Auth::user()->role == 'admin')
                    <div onclick="switchTab('rekap')" class="bg-orange-500 p-4 md:p-5 rounded-[2rem] shadow-xl text-white text-right cursor-pointer hover:bg-orange-600 hover:scale-105 transition-all">
                        <span class="text-[9px] uppercase font-black opacity-70 block tracking-widest text-xs">Total Omzet Hari Ini ⬇️</span>
            ...
            <div id="section-rekap" class="hidden space-y-8 print-area">
                <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 no-print flex flex-wrap gap-4 items-end">
                    <div class="flex-1 min-w-[150px]">
                        <label class="text-[10px] font-black text-gray-400 uppercase mb-2 block">Pilih Rentang Tanggal</label>
                    ...
                    <button onclick="filterReport()" class="bg-gray-900 text-white px-8 py-3.5 rounded-xl font-black text-xs uppercase hover:bg-orange-600 transition-all shadow-lg">Terapkan Filter</button>
                    <button onclick="window.print()" class="bg-blue-500 text-white px-8 py-3.5 rounded-xl font-black text-xs uppercase shadow-lg hover:bg-blue-600 transition-all">🖨️ PDF</button>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                    <div class="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm"><p class="text-[10px] font-black text-gray-400 uppercase mb-2">Saldo Tunai</p><p id="rekap-cash-display" class="text-2xl font-black text-green-600 tracking-tighter">Rp {{ number_format($riwayat->where('metode_pembayaran','Cash')->sum('total_harga')) }}</p></div>
                    <div class="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm"><p class="text-[10px] font-black text-gray-400 uppercase mb-2">Saldo Digital</p><p id="rekap-digital-display" class="text-2xl font-black text-blue-600 tracking-tighter">Rp {{ number_format($riwayat->where('metode_pembayaran','!=','Cash')->sum('total_harga')) }}</p></div>
                    <div onclick="document.getElementById('report-harian-detail').scrollIntoView({behavior:'smooth'})" class="bg-orange-500 p-8 rounded-[3rem] shadow-xl text-white cursor-pointer hover:bg-orange-600 hover:scale-105 transition-all">
                ...
                <div id="audit-result-area" class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                    ...
                    <div id="audit-content"><p class="text-center text-xs text-gray-400 font-black py-10 italic uppercase tracking-widest">Belum ada shift ditutup (🔒)</p></div>
                </div>

                <div id="report-harian-detail" class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left min-w-[600px]">
                            <thead><tr class="text-gray-400 text-[10px] uppercase font-black border-b border-gray-50"><th class="pb-4 px-2">Waktu</th><th class="pb-4 px-2">Pelanggan</th><th class="pb-4 px-2">Detail Item</th><th class="pb-4 px-2">Metode</th><th class="pb-4 px-2 text-right">Total</th><th class="pb-4 text-center">Void</th></tr></thead>
                            <tbody id="report-table-body" class="divide-y divide-gray-50 text-xs font-bold text-gray-600">
                                @foreach($riwayat as $trx)
                                <tr class="group hover:bg-orange-50 transition-colors">
                                    <td class="py-5 text-gray-400 px-2">{{ $trx->created_at->format('H:i') }}</td>
                                    ...
                                    <td class="py-5 px-2 text-[9px] font-black uppercase italic">{{ $trx->metode_pembayaran }}</td>
        ...
        <div class="w-full md:w-96 bg-white p-6 shadow-2xl flex flex-col border-l border-gray-100 h-full overflow-y-auto z-20 no-scrollbar no-print">
            ...
            <div id="cart-container" class="space-y-3 mb-6 flex-1"><p class="py-10 text-center font-black opacity-20 uppercase text-[10px]">Pilih Menu...</p></div>
            <div class="border-t border-gray-50 pt-4 space-y-4">
                ...
                <div class="flex flex-col space-y-3">
                    <select id="payment-method" class="w-full p-4 bg-gray-50 border rounded-xl outline-none text-sm font-bold text-gray-700 shadow-sm appearance-none cursor-pointer">
                    ...
                    <button id="btn-checkout" onclick="performCheckout()" class="w-full bg-orange-500 text-white py-5 rounded-3xl font-black text-sm shadow-lg hover:bg-orange-600 active:scale-95 transition-all disabled:opacity-30 cursor-pointer" disabled>BAYAR SEKARANG</button>
                </div>
            </div>
        </div>
    </div>

    <div id="inventory-modal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-[9000] p-4 backdrop-blur-sm no-print">
        <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-cart-item-enter">
            <div class="bg-orange-500 p-6 text-white flex justify-between items-center">
                <h2 class="text-xl font-black uppercase tracking-tight">📦 Gudang Stok</h2>
            ...
            <div class="p-6 pt-0"><button onclick="document.getElementById('inventory-modal').classList.replace('flex','hidden')" class="w-full bg-gray-900 text-white py-4 rounded-2xl font-black uppercase text-xs hover:bg-black transition-all cursor-pointer">Selesai Memeriksa</button></div>
        </div>
    </div>

    <div id="receipt-modal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-[9999] p-4 backdrop-blur-sm">
    ...
    <script>
        ...
        // 1. FORCED SHIFT SYSTEM & GREETING (FIXED OWNER DISAPA AULIA)
        ...
        function updateUIWithActiveShift() {
            ...
            if(isOwner) {
                greeting.innerHTML = "HAI OWNER! ☕"; // KUNCI MATI
        ...
        function startShift() {
            ...
            if(!name || !amount) return alert('⚠️ Wajib isi Nama & Modal Laci!');
        ...
        function endShift() {
            ...
            let res = `<div class="text-left space-y-2 p-6 bg-gray-50 rounded-3xl border-2 ${diff < 0 ? 'border-red-100' : 'border-green-100'}"><h3 class="font-black uppercase text-[10px] text-center mb-4 underline italic">Audit Shift: ${name}</h3><div class="flex justify-between text-xs"><span>Kas Awal:</span><span class="font-bold">Rp ${opening.toLocaleString()}</span></div><div class="flex justify-between text-xs"><span>Omzet Tunai:</span><span class="font-bold">Rp ${sales.toLocaleString()}</span></div><hr class="border-dashed"><div class="flex justify-between text-xs font-black"><span>Harusnya:</span><span>Rp ${expected.toLocaleString()}</span></div><div class="flex justify-between text-xs font-black text-blue-600"><span>Fisik Laci:</span><span>Rp ${closing.toLocaleString()}</span></div><hr class="border-dashed border-gray-300"><div class="flex justify-between ${diff < 0 ? 'text-red-500' : 'text-green-600'} font-black uppercase text-sm"><span>Status:</span><span>${diff < 0 ? 'MINUS Rp ' + Math.abs(diff).toLocaleString() : 'COCOK ✨'}</span></div></div>`;
        ...
        function switchTab(t) {
            ...
            if(t==='pos'){ pos.classList.remove('hidden'); rekap.classList.add('hidden'); bP.classList.add('border-b-4','border-orange-500','text-orange-600'); bR.classList.remove('border-b-4','border-orange-500','text-orange-600'); }
                ...
                const last = localStorage.getItem('last_audit_html'); if(last) document.getElementById('audit-content').innerHTML = last;
                // AUTO LOAD TODAY'S TOTALS ON TAB OPEN
                filterReportToday();
            }
        }

        function filterReportToday() {
            const today = new Date().toISOString().split('T')[0];
            fetch(`/get-report?start_date=${today}&end_date=${today}`)
                .then(r => r.json()).then(data => {
        ...
        function voidTransaction(id) { if(confirm('Hapus transaksi? Stok kembali.')) fetch(`/transaksi/${id}`, { method: "DELETE", headers: { "X-CSRF-TOKEN": "{{ csrf_token() }}" } }).then(() => window.location.reload()); }
