...
<head>
    ...
    <title>Street Coffee POS - Owner Priority Edition</title>
...
<body class="bg-gray-100 font-sans text-gray-900">
    ...
    <div id="shift-modal" class="fixed inset-0 bg-black/95 hidden items-center justify-center z-[700] p-4 backdrop-blur-2xl no-print">
    ...
    <div id="close-shift-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-[600] p-4 backdrop-blur-md no-print">
    ...
    <div class="flex flex-col md:flex-row min-h-screen md:h-screen overflow-hidden bg-gray-100">
        ...
        <div class="w-full md:w-20 bg-white shadow-sm md:shadow-lg flex flex-row md:flex-col items-center justify-evenly md:justify-start py-4 md:py-6 space-x-2 md:space-x-0 md:space-y-8 z-30 sticky top-0 md:relative no-print">
            ...
            <div class="md:mt-auto flex flex-row md:flex-col items-center space-x-4 md:space-x-0 md:space-y-4">
                ...
                <button id="open-inventory" class="relative p-2 md:p-3 text-gray-400 hover:text-orange-500 transition-colors text-xl cursor-pointer" title="Gudang">
                    📦
        ...
        <div class="flex-1 p-4 md:p-8 overflow-y-auto h-full bg-gray-50/50 custom-scroll">
            ...
            <div id="section-pos" class="space-y-8 no-print">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p id="active-cashier-label" class="text-gray-400 text-[10px] font-black uppercase tracking-widest italic">Street Coffee Premium POS</p>
                    </div>
                    @if(Auth::user()->role == 'admin')
                    <div onclick="switchTab('rekap')" class="bg-orange-500 p-4 md:p-5 rounded-[2rem] shadow-xl text-white text-right cursor-pointer hover:bg-orange-600 hover:scale-105 transition-all">
                        <span class="text-[9px] uppercase font-black opacity-70 block">Omzet Hari Ini ⬇️</span>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left min-w-[600px]">
                            <thead><tr class="text-gray-400 text-[10px] uppercase font-black border-b border-gray-50"><th class="pb-4 px-2">Waktu</th><th class="pb-4 px-2">Pelanggan</th><th class="pb-4 px-2">Item</th><th class="pb-4 px-2 text-right">Total</th><th class="pb-4 text-center">Void</th></tr></thead>
                            <tbody class="divide-y divide-gray-50 text-xs font-bold text-gray-600">
                                ...
                                <tr class="group hover:bg-orange-50 transition-colors">
                                    ...
                                    <td class="py-5 text-gray-500 italic px-2 text-[10px]">{{ $trx->item_list }}</td>
            ...
            <div id="section-rekap" class="hidden space-y-6 print-area">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                    <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm text-center"><p class="text-[10px] font-black text-gray-400 uppercase mb-2">Saldo Tunai</p><p id="rekap-cash-display" class="text-2xl font-black text-green-600 tracking-tighter">Rp 0</p></div>
                    <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm text-center"><p class="text-[10px] font-black text-gray-400 uppercase mb-2">Saldo Digital</p><p id="rekap-digital-display" class="text-2xl font-black text-blue-600 tracking-tighter">Rp 0</p></div>
                ...
                <div id="audit-result-area" class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button onclick="localStorage.removeItem('shift_active'); window.location.reload();" class="text-[9px] font-black text-red-400 uppercase border border-red-50 px-3 py-1 rounded-lg no-print cursor-pointer">⚠️ Reset Shift</button>
                    </div>
                    <div id="audit-content"><p class="text-center text-xs text-gray-400 font-black py-10">Belum ada shift ditutup.</p></div>
                </div>
            </div>
        </div>
        ...
        <div class="w-full md:w-96 bg-white p-6 shadow-2xl flex flex-col border-l border-gray-100 h-full overflow-y-auto z-20 no-scrollbar no-print">
            ...
            <div class="border-t border-gray-50 pt-4 space-y-4">
                <div class="flex justify-between items-center"><span class="text-xs font-black text-gray-400 uppercase">Total Tagihan</span><span id="cart-total" class="text-2xl font-black text-orange-600 tracking-tighter">Rp 0</span></div>
                <div class="flex flex-col space-y-3">
                    <select id="payment-method" class="w-full p-3 bg-gray-50 border rounded-xl outline-none text-sm font-bold text-gray-700 shadow-sm appearance-none cursor-pointer">
                    ...
                    <button id="btn-checkout" onclick="performCheckout()" class="w-full bg-orange-500 text-white py-4 rounded-2xl font-black text-sm shadow-lg hover:bg-orange-600 active:scale-95 transition-all disabled:opacity-30 cursor-pointer" disabled>BAYAR SEKARANG</button>
                </div>
            </div>
        </div>
    </div>

    <div id="inventory-modal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-[800] p-4 backdrop-blur-sm no-print">
            ...
            <div class="p-6">
                ...
                <div id="inventory-list" class="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scroll no-scrollbar">
                    ...
                    <div class="inventory-item p-4 bg-gray-50 rounded-2xl transition-all border border-transparent hover:border-orange-100" data-name="{{ strtolower($bahan->nama_bahan) }}">
                        ...
                        @if(Auth::user()->role == 'admin')
                        <div class="flex space-x-2"><input type="number" id="restock-qty-{{ $bahan->id }}" placeholder="+ stok" class="flex-1 p-2 bg-white border border-gray-200 rounded-lg outline-none text-[10px] font-bold"><button onclick="restockItem({{ $bahan->id }})" class="bg-gray-900 text-white px-3 py-2 rounded-lg text-[9px] font-black uppercase hover:bg-orange-600 transition-all cursor-pointer">Update</button></div>
                        @endif
                    </div>
                    @endforeach
                </div>
            </div>
            <div class="p-6 pt-0"><button onclick="document.getElementById('inventory-modal').classList.replace('flex','hidden')" class="w-full bg-gray-900 text-white py-3 rounded-xl font-black uppercase text-xs hover:bg-black transition-all cursor-pointer">Selesai Cek</button></div>
        </div>
    </div>

    <div id="receipt-modal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-[900] p-4 backdrop-blur-sm">
        <div id="modal-content" class="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300 text-center text-xs font-mono">
            ...
            <div class="p-8 space-y-4 text-left">
                <div class="flex justify-between border-b border-dashed pb-2"><span>Pelanggan:</span><span id="receipt-customer" class="font-black uppercase"></span></div>
    ...
    <script>
        ...
        // 1. FIXED FORCED LOGIN MODAL (BYPASS FOR OWNER)
        ...
        function performCheckout() {
            ...
            if(!n) return alert('⚠️ Mohon isi Nama Pelanggan!'); btnPay.disabled = true;
        ...
        function endShift() {
            ...
            let res = `<div class="text-left space-y-2 p-6 bg-gray-50 rounded-3xl border-2 ${diff < 0 ? 'border-red-100' : 'border-green-100'}"><h3 class="font-black uppercase text-[10px] text-center mb-4 underline">Audit Shift: ${name}</h3><div class="flex justify-between text-xs"><span>Kas Awal:</span><span class="font-bold">Rp ${opening.toLocaleString()}</span></div><div class="flex justify-between text-xs"><span>Omzet Tunai:</span><span class="font-bold">Rp ${sales.toLocaleString()}</span></div><hr class="border-dashed border-gray-300"><div class="flex justify-between text-xs font-black"><span>Seharusnya:</span><span>Rp ${expected.toLocaleString()}</span></div><div class="flex justify-between text-xs font-black text-blue-600"><span>Fisik Laci:</span><span>Rp ${closing.toLocaleString()}</span></div><hr class="border-dashed border-gray-300"><div class="flex justify-between ${diff < 0 ? 'text-red-500' : 'text-green-600'} font-black uppercase text-sm"><span>Status:</span><span>${diff < 0 ? 'MINUS Rp ' + Math.abs(diff).toLocaleString() : 'COCOK ✨'}</span></div></div>`;
