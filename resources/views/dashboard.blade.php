...
<head>
    ...
    <title>Street Coffee POS - Master Sempurna</title>
...
<body class="bg-gray-100 font-sans text-gray-900">
    ...
    <div id="shift-modal" class="fixed inset-0 bg-black/90 hidden items-center justify-center z-[200] p-4 backdrop-blur-xl no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl border-4 border-orange-500">
            ...
            <p class="text-[10px] text-gray-400 font-bold mb-8 italic uppercase">Isi Nama & Modal untuk Masuk Dashboard</p>
            <div class="space-y-4 mb-8">
                <div class="text-left">
                    <label class="text-[9px] font-black text-gray-400 uppercase ml-2 mb-1 block">Nama Kasir Bertugas</label>
                    <input type="text" id="cashier-name-input" placeholder="Masukkan Nama..." class="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none text-center font-bold focus:ring-4 focus:ring-orange-200">
                </div>
                <div class="text-left">
                    <label class="text-[9px] font-black text-gray-400 uppercase ml-2 mb-1 block">Modal Awal Laci (Rp)</label>
                    <input type="number" id="opening-cash" placeholder="0" class="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl outline-none text-center text-xl font-black text-orange-600 focus:ring-4 focus:ring-orange-200">
                </div>
            </div>
    ...
    <div id="close-shift-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-[150] p-4 backdrop-blur-md no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl relative">
            ...
            <h2 class="text-xl font-black text-gray-800 uppercase mb-2 tracking-tighter">Selesai Shift</h2>
    ...
    <div class="flex flex-col md:flex-row min-h-screen md:h-screen overflow-hidden bg-gray-100">
        ...
        <div class="flex-1 p-4 md:p-8 overflow-y-auto h-full bg-gray-50/50 custom-scroll">
            ...
            <div id="section-pos" class="space-y-8 no-print">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 id="main-greeting" class="text-xl md:text-2xl font-black text-gray-800 tracking-tight uppercase">Street Coffee</h1>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                    ...
                    <div class="menu-item bg-white rounded-[2rem] shadow-sm hover:shadow-xl transition-all p-3 md:p-4 group border border-transparent hover:border-orange-100" data-category="{{ $menu->kategori }}">
                        ...
                        <h3 class="font-bold text-gray-800 mb-1 text-sm truncate uppercase">{{ $menu->nama_menu }}</h3>
                        <div class="flex justify-between items-center">
                ...
                <div class="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left min-w-[500px]">
                            <thead><tr class="text-gray-400 text-[10px] uppercase font-black border-b border-gray-50"><th class="pb-4 px-2">Waktu</th><th class="pb-4 px-2">Customer</th><th class="pb-4 px-2 text-right">Total</th><th class="pb-4 text-center">Aksi</th></tr></thead>
            ...
            <div id="section-rekap" class="hidden space-y-6 print-area">
                ...
                <div id="audit-result-area" class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button onclick="if(confirm('Reset shift?')) {localStorage.removeItem('shift_active'); window.location.reload();}" class="text-[9px] font-black text-red-400 uppercase border border-red-50 px-3 py-1 rounded-lg no-print">⚠️ Reset Shift</button>
                    </div>
        ...
        <div class="w-full md:w-96 bg-white p-6 shadow-2xl flex flex-col border-l border-gray-100 h-full overflow-y-auto z-20 no-scrollbar no-print">
            ...
            <div id="cart-container" class="space-y-3 mb-6 flex-1"><p class="py-10 text-center font-black opacity-20 uppercase text-[10px]">Keranjang Kosong</p></div>
            <div class="border-t border-gray-50 pt-4 space-y-4">
                ...
                <div class="flex flex-col space-y-3">
                    ...
                    <button id="btn-checkout" onclick="performCheckout()" class="w-full bg-orange-500 text-white py-4 rounded-2xl font-black text-sm shadow-lg hover:bg-orange-600 disabled:opacity-30 disabled:bg-gray-200 transition-all" disabled>BAYAR SEKARANG</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="inventory-modal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-[110] p-4 backdrop-blur-sm no-print">
        <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-cart-item-enter">
            <div class="bg-orange-500 p-6 text-white flex justify-between items-center"><h2 class="text-xl font-black uppercase tracking-tight">📦 Gudang</h2><button onclick="document.getElementById('inventory-modal').classList.replace('flex','hidden')" class="text-3xl font-bold hover:text-black">&times;</button></div>
            <div class="p-6">
                <div class="relative mb-6">
                    <input type="text" id="search-inventory" placeholder="Cari bahan kopi..." class="w-full p-3 pl-10 bg-gray-50 border rounded-xl outline-none text-sm font-bold focus:ring-2 focus:ring-orange-500 transition-all shadow-sm">
    ...
    <script>
        ...
        // 1. FORCED SHIFT SYSTEM (WAJIB ISI!)
        ...
        function updateUIWithActiveShift() {
            ...
            if(name) {
                // Tampilan sapaan personal tanpa double kata "Kasir"
                greeting.innerHTML = `Halo, ${name}! ✨`;
                document.getElementById('active-cashier-label').innerText = "Kasir Aktif: " + name;
            }
        }
        ...
        function startShift() {
            ...
            if(!name || !amount) return alert('⚠️ Tiara, wajib isi Nama & Modal dulu ya!');
            
            localStorage.setItem('shift_active', 'true');
            localStorage.setItem('current_cashier', name);
            localStorage.setItem('opening_cash', amount);
            localStorage.setItem('total_cash_sales', '0');
        ...
        function endShift() {
            ...
            let res = `<div class="text-left space-y-2 p-6 bg-gray-50 rounded-3xl border-2 ${diff < 0 ? 'border-red-100' : 'border-green-100'}">
                <h3 class="font-black uppercase text-[10px] text-center mb-4 underline">Audit Shift: ${name}</h3>
            ...
            localStorage.setItem('last_audit_html', res); 
        ...
        function performCheckout() {
            ...
            if(!n) return alert('Isi Nama Pelanggan!'); btnPay.disabled = true;
            ...
            .then(r => r.json()).then(data => {
                if(data.success) {
                    ...
                    document.getElementById('receipt-customer').innerText = n; document.getElementById('receipt-method').innerText = m; document.getElementById('receipt-total').innerText = "Rp " + t.toLocaleString();
                    ...
                    document.getElementById('receipt-items').innerHTML = itemsHtml; document.getElementById('receipt-modal').classList.replace('hidden','flex');
        ...
        function updateCartUI() {
            ...
            cart.forEach((item, index) => { total += (item.price * item.qty); html += `<div class="flex justify-between items-center bg-gray-50 p-3 rounded-2xl border mb-2 cart-item-enter"><div class="flex-1 pr-2"><p class="font-bold text-gray-800 text-[11px] truncate uppercase">${item.name}</p><p class="text-[9px] text-gray-400 font-bold">${item.qty}x @ Rp ${item.price.toLocaleString()}</p></div><div class="flex items-center space-x-3"><span class="text-xs font-black text-orange-600">Rp ${(item.price * item.qty).toLocaleString()}</span><button onclick="cart.splice(${index},1);updateCartUI();" class="text-gray-300 hover:text-red-500 font-bold text-lg">&times;</button></div></div>`; });
        ...
        document.getElementById('search-inventory').addEventListener('input', function() {
            ...
            document.querySelectorAll('.inventory-item').forEach(item => { item.style.display = item.getAttribute('data-name').includes(q) ? 'block' : 'none'; });
        });
