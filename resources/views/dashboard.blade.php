...
<head>
    ...
    <title>Street Coffee POS - Master Final Absolute</title>
...
<body class="bg-gray-100 font-sans text-gray-900 scroll-smooth">
    ...
    <div id="shift-modal" class="fixed inset-0 bg-black/95 hidden items-center justify-center z-[10000] p-4 backdrop-blur-2xl no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl border-4 border-orange-500">
            ...
            <div class="space-y-5 mb-10">
                <div class="text-left">
                    ...
                    <input type="text" id="cashier-name-input" placeholder="Nama Kasir..." class="w-full p-5 bg-gray-50 border border-gray-100 rounded-3xl outline-none text-center font-black text-gray-700 focus:ring-4 focus:ring-orange-200 text-lg">
                </div>
                <div class="text-left">
    ...
    <div class="flex flex-col md:flex-row min-h-screen md:h-screen overflow-hidden bg-gray-100">
        ...
        <div class="w-full md:w-20 bg-white shadow-sm md:shadow-lg flex flex-row md:flex-col items-center py-4 md:py-6 overflow-x-auto md:overflow-x-visible no-scrollbar z-30 sticky top-0 md:relative no-print px-4 md:px-0">
            <div class="text-orange-600 font-bold text-2xl hidden md:block md:mb-8">☕</div>
            <div class="flex flex-row md:flex-col items-center space-x-3 md:space-x-0 md:space-y-6 min-w-max md:min-w-0">
                <button class="filter-btn p-2 md:p-3 bg-orange-100 text-orange-600 rounded-xl text-sm font-bold" data-target="all">All</button>
                <button class="filter-btn p-2 md:p-3 text-gray-400 hover:text-orange-600 rounded-xl text-sm font-bold" data-target="Coffee">Coffee</button>
                <button class="filter-btn p-2 md:p-3 text-gray-400 hover:text-orange-600 rounded-xl text-sm font-bold" data-target="Non-Coffee">Non-Coffee</button>
                <button class="filter-btn p-2 md:p-3 text-gray-400 hover:text-orange-600 rounded-xl text-sm font-bold" data-target="Snack">Snack</button>
                <button onclick="openCloseShiftModal()" class="p-2 md:p-3 text-gray-400 hover:text-red-500 text-xl cursor-pointer" title="Tutup Shift">🔒</button>
                <button onclick="openInventory()" class="relative p-2 md:p-3 text-gray-400 hover:text-orange-500 text-xl cursor-pointer">
                    📦
                    @php 
                        // FIXED STOCK LOGIC: Perbandingan Angka Murni
                        $lowStockCount = $stokBahan->filter(function($item) { return (int)$item->stok <= (int)$item->min_stok; })->count();
                    @endphp
                    @if($lowStockCount > 0)
                ...
                <form id="logout-form" action="{{ route('logout') }}" method="POST" class="inline">@csrf 
                    <button type="submit" class="p-2 md:p-3 text-gray-300 hover:text-red-500 transition-colors cursor-pointer text-xl">🚪</button>
                </form>
            </div>
        </div>
        ...
        <div class="flex-1 p-4 md:p-8 overflow-y-auto h-full bg-gray-50/50 custom-scroll">
            ...
            <div class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-xl flex items-center justify-between shadow-sm no-print">
                <div class="flex items-center">
                    ...
                    <p class="text-xs font-black text-red-700 uppercase tracking-tighter">Owner, ada {{ $lowStockCount }} bahan menipis!</p>
                </div>
            ...
            <div id="section-pos" class="space-y-8 no-print">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    @if(Auth::user()->role == 'admin')
                    <div onclick="switchTab('rekap')" class="bg-orange-500 p-4 md:p-5 rounded-[2rem] shadow-xl text-white text-right cursor-pointer hover:bg-orange-600 transition-all w-full md:w-auto">
                ...
                <div id="daily-history-table" class="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-gray-100">
                        ...
                        <table class="w-full text-left min-w-[650px]">
                            ...
                            <tbody class="divide-y divide-gray-50 text-xs font-bold text-gray-600">
                                ...
                                <tr class="group hover:bg-orange-50 transition-colors">
                                    ...
                                    <td class="py-5 px-2 text-[9px] font-black uppercase italic text-blue-500">{{ $trx->metode_pembayaran }}</td>
            ...
            <div id="section-rekap" class="hidden space-y-6 print-area">
                ...
                <div id="audit-result-area" class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button onclick="localStorage.removeItem('last_audit_html'); window.location.reload();" class="text-[9px] font-black text-red-400 uppercase border border-red-50 px-3 py-1 rounded-lg no-print cursor-pointer">⚠️ Reset Audit</button>
                    </div>
                    <div id="audit-content"><p class="text-center text-xs text-gray-400 font-black py-10 uppercase italic tracking-widest">Belum Ada Laporan Audit Tersimpan.</p></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-cart-item-enter">
            ...
            <div class="p-6">
                <div class="relative mb-6">
                    <input type="text" id="search-inventory" placeholder="Cari bahan..." class="w-full p-4 pl-12 bg-gray-50 border rounded-2xl outline-none text-sm font-bold focus:ring-2 focus:ring-orange-500">
    ...
    <div id="receipt-modal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-[9500] p-4 backdrop-blur-sm">
        <div id="modal-content" class="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300 text-center text-xs font-mono">
            ...
            <div class="p-8 pt-0"><button onclick="window.location.reload()" class="w-full bg-gray-900 text-white py-5 rounded-3xl font-black hover:bg-black transition-all uppercase tracking-widest shadow-xl cursor-pointer">Selesai & Tutup</button></div>
        </div>
    </div>
    ...
    <script>
        ...
        function updateUIWithActiveShift() {
            ...
            if(isOwner) {
                ...
                document.getElementById('active-cashier-label').innerText = name ? "Shift Berjalan: " + name : "Belum Ada Kasir Bertugas";
            } else if(name) {
        ...
        // 3. AUDIT & LOGOUT (FIXED Memori Laporan)
        function endShift() {
            const name = localStorage.getItem('current_cashier'), 
                  opening = parseFloat(localStorage.getItem('opening_cash')) || 0, 
                  sales = parseFloat(localStorage.getItem('total_cash_sales')) || 0, 
                  closing = parseFloat(document.getElementById('closing-cash').value) || 0, 
                  expected = opening + sales, diff = closing - expected;
            ...
            let res = `<div class="text-left space-y-2 p-6 bg-gray-50 rounded-3xl border-2 ${diff < 0 ? 'border-red-100' : 'border-green-100'}">
                ...
                <hr class="border-dashed border-gray-300"><div class="flex justify-between text-xs font-black"><span>Harusnya:</span><span>Rp ${expected.toLocaleString()}</span></div>
            ...
            localStorage.setItem('last_audit_html', res); // SIMPAN LAPORAN SEBELUM LOGOUT
            alert('Laporan Audit Shift ' + name + ' Berhasil Disimpan. Sistem akan Logout.');
        ...
        function switchTab(t) {
            ...
            if(t==='pos'){ pos.classList.remove('hidden'); rekap.classList.add('hidden'); bP.classList.add('border-b-4','border-orange-500','text-orange-600'); bR.classList.remove('border-b-4','border-orange-500','text-orange-600'); }
                ...
                const auditHtml = localStorage.getItem('last_audit_html');
                if(auditHtml) document.getElementById('audit-content').innerHTML = auditHtml;
                fetchTodaySummary();
            }
        }
