...
<body class="bg-gray-100 font-sans text-gray-900">
    ...
    <div id="shift-modal" class="fixed inset-0 bg-black/95 hidden items-center justify-center z-[700] p-4 backdrop-blur-2xl no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl border-4 border-orange-500">
            ...
            <p class="text-[10px] text-gray-400 font-bold mb-10 italic uppercase">Identitas Wajib Sebelum Operasional</p>
    ...
    <div class="flex flex-col md:flex-row min-h-screen md:h-screen overflow-hidden bg-gray-100">
        ...
        <div class="w-full md:w-20 bg-white shadow-sm md:shadow-lg flex flex-row md:flex-col items-center justify-evenly md:justify-start py-4 md:py-6 space-x-2 md:space-x-0 md:space-y-8 z-30 sticky top-0 md:relative no-print">
            ...
            <div class="md:mt-auto flex flex-row md:flex-col items-center space-x-4 md:space-x-0 md:space-y-4">
                <button onclick="openCloseShiftModal()" class="p-2 md:p-3 text-gray-400 hover:text-red-500 transition-colors text-xl cursor-pointer" title="Tutup Kasir">🔒</button>
            ...
            <div id="section-pos" class="space-y-8 no-print">
                <div class="flex justify-between items-center">
                    ...
                    <div onclick="switchTab('rekap')" class="bg-orange-500 p-4 md:p-5 rounded-[2rem] shadow-xl text-white text-right cursor-pointer hover:bg-orange-600 hover:scale-105 transition-all">
                        <span class="text-[9px] uppercase font-black opacity-70 block tracking-widest">Total Pendapatan ⬇️</span>
                        ...
                        <table class="w-full text-left min-w-[600px]">
                            <thead>
                                <tr class="text-gray-400 text-[10px] uppercase font-black border-b border-gray-50">
                                    <th class="pb-4 px-2">Waktu</th><th class="pb-4 px-2">Pelanggan</th><th class="pb-4 px-2">Detail Item</th><th class="pb-4 px-2 text-right">Total</th><th class="pb-4 text-center">Void</th>
                                </tr>
                            </thead>
        ...
        <div class="w-full md:w-96 bg-white p-6 shadow-2xl flex flex-col border-l border-gray-100 h-full overflow-y-auto z-20 no-scrollbar no-print">
            ...
            <div class="border-t border-gray-50 pt-4 space-y-4">
                ...
                <div class="flex flex-col space-y-3">
                    ...
                    <button id="btn-checkout" onclick="performCheckout()" class="w-full bg-orange-500 text-white py-5 rounded-3xl font-black text-sm shadow-xl hover:bg-orange-600 active:scale-95 disabled:opacity-30 transition-all uppercase tracking-widest cursor-pointer" disabled>BAYAR SEKARANG</button>
                </div>
            </div>
        </div>
    </div>

    <div id="inventory-modal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-[550] p-4 backdrop-blur-sm no-print">
    ...
    <div id="receipt-modal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-[800] p-4 backdrop-blur-sm">
    ...
    <script>
        ...
        function endShift() {
            ...
            let res = `<div class="text-left space-y-2 p-6 bg-gray-50 rounded-3xl border-2 ${diff < 0 ? 'border-red-100' : 'border-green-100'}"><h3 class="font-black uppercase text-[10px] text-center mb-4 underline">Audit Shift: ${name}</h3><div class="flex justify-between text-xs"><span>Kas Awal:</span><span class="font-bold">Rp ${opening.toLocaleString()}</span></div><div class="flex justify-between text-xs"><span>Omzet Tunai:</span><span class="font-bold">Rp ${sales.toLocaleString()}</span></div><hr class="border-dashed"><div class="flex justify-between text-xs font-black"><span>Harusnya:</span><span>Rp ${expected.toLocaleString()}</span></div><div class="flex justify-between text-xs font-black text-blue-600"><span>Fisik Laci:</span><span>Rp ${closing.toLocaleString()}</span></div><hr class="border-dashed border-gray-300"><div class="flex justify-between ${diff < 0 ? 'text-red-500' : 'text-green-600'} font-black uppercase text-sm"><span>Status:</span><span>${diff < 0 ? 'MINUS Rp ' + Math.abs(diff).toLocaleString() : 'COCOK ✨'}</span></div></div>`;
