...
<head>
    ...
    <title>Street Coffee POS - Sempurna Final</title>
...
<body class="bg-gray-100 font-sans text-gray-900">
    ...
    <div id="shift-modal" class="fixed inset-0 bg-black/95 hidden items-center justify-center z-[300] p-4 backdrop-blur-2xl no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl border-4 border-orange-500">
            ...
            <p class="text-[10px] text-gray-400 font-bold mb-10 italic uppercase tracking-widest">Identitas Wajib Sebelum Berjualan</p>
            <div class="space-y-5 mb-10">
                <div class="text-left">
                    <label class="text-[10px] font-black text-gray-400 uppercase ml-2 mb-1 block">Siapa Nama Kasir?</label>
    ...
    <div id="close-shift-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-[250] p-4 backdrop-blur-md no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl relative border-4 border-gray-900">
            <button onclick="document.getElementById('close-shift-modal').classList.replace('flex','hidden')" class="absolute top-6 right-8 text-gray-300 hover:text-red-500 font-bold text-3xl">&times;</button>
            <div class="text-5xl mb-4">🔒</div>
            <h2 class="text-xl font-black text-gray-800 uppercase mb-2">Tutup Kasir</h2>
            <p class="text-xs text-gray-400 font-bold mb-8 italic text-red-500 uppercase">Input Uang Fisik di Laci!</p>
            ...
            <button onclick="endShift()" class="w-full bg-gray-900 text-white py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl hover:bg-black active:scale-95 transition-all">Kirim Laporan Audit</button>
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row min-h-screen md:h-screen overflow-hidden bg-gray-100">
        ...
        <div class="flex-1 p-4 md:p-8 overflow-y-auto h-full bg-gray-50/50 custom-scroll">
            ...
            <div id="section-pos" class="space-y-8 no-print">
                <div class="flex justify-between items-center">
                    <div>
                        <h1 id="main-greeting" class="text-xl md:text-2xl font-black text-gray-800 tracking-tight uppercase">Menunggu Kasir...</h1>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
                    <h2 class="text-lg font-black text-gray-800 mb-6 uppercase tracking-tighter">📋 Riwayat Orang Pesan (Hari Ini)</h2>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left min-w-[600px]">
                            <thead>
                                <tr class="text-gray-400 text-[10px] uppercase font-black border-b border-gray-50">
                                    <th class="pb-4 px-2">Waktu</th>
                                    <th class="pb-4 px-2">Nama Pelanggan</th>
                                    <th class="pb-4 px-2">Apa Saja Yang Dibeli?</th>
                                    <th class="pb-4 px-2">Bayar Pakai</th>
                                    <th class="pb-4 px-2 text-right">Total Bayar</th>
                                    <th class="pb-4 text-center">Void</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50 text-xs">
                                ...
                                <tr class="group hover:bg-orange-50 transition-colors">
                                    ...
                                    <td class="py-5 text-gray-500 italic px-2 leading-relaxed">{{ $trx->item_list }}</td>
                                    <td class="py-5 px-2 font-black text-blue-500 uppercase italic text-[10px]">{{ $trx->metode_pembayaran }}</td>
                                    <td class="py-5 text-right font-black text-orange-600 px-2">Rp {{ number_format($trx->total_harga) }}</td>
                                    <td class="py-5 text-center">
                                        @if(Auth::user()->role == 'admin')
                                        <button onclick="voidTransaction({{ $trx->id }})" class="text-gray-200 hover:text-red-500 transition-colors text-lg">🗑️</button>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
            ...
            <div id="section-rekap" class="hidden space-y-6 print-area">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                    <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm"><p class="text-[10px] font-black text-gray-400 uppercase mb-2">Saldo Tunai (Laci)</p><p id="rekap-cash-display" class="text-2xl font-black text-green-600 tracking-tighter">Rp 0</p></div>
                    <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm"><p class="text-[10px] font-black text-gray-400 uppercase mb-2">Saldo Digital (Bank)</p><p id="rekap-digital-display" class="text-2xl font-black text-blue-600 tracking-tighter">Rp 0</p></div>
        ...
        <div class="w-full md:w-96 bg-white p-6 shadow-2xl flex flex-col border-l border-gray-100 h-full overflow-y-auto z-20 no-scrollbar no-print">
            <h2 class="text-xl font-black text-gray-800 mb-6 uppercase tracking-tighter">🛒 Pesanan</h2>
            <div class="space-y-4 mb-4"><input type="text" id="customer-name" placeholder="Nama Pelanggan..." class="w-full p-4 bg-gray-50 border rounded-2xl outline-none text-sm font-bold focus:ring-2 focus:ring-orange-500"></div>
            <div id="cart-container" class="space-y-3 mb-6 flex-1"><p class="py-10 text-center font-black opacity-20 uppercase text-[10px] tracking-widest italic">Belum ada menu dipilih...</p></div>
            <div class="border-t border-gray-50 pt-4 space-y-4">
                ...
                <div class="flex flex-col space-y-3">
                    <select id="payment-method" class="w-full p-4 bg-gray-50 border rounded-2xl outline-none text-sm font-bold text-gray-700 shadow-sm appearance-none">
                    ...
                    <div id="cash-calculator" class="p-4 bg-orange-50 rounded-2xl border border-orange-100">
                        <div class="mb-3 text-center"><label class="text-[10px] font-black text-orange-400 uppercase block mb-1">Uang Diterima</label><input type="number" id="cash-amount" placeholder="0" class="w-full p-3 bg-white border rounded-xl outline-none text-lg font-black text-gray-800 text-center"></div>
                        <div class="flex justify-between items-center"><span class="text-[10px] font-black text-gray-400 uppercase">Kembalian</span><span id="change-amount" class="text-sm font-black text-orange-600">Rp 0</span></div>
                    </div>
                    <button id="btn-checkout" onclick="performCheckout()" class="w-full bg-orange-500 text-white py-5 rounded-3xl font-black text-sm shadow-xl hover:bg-orange-600 active:scale-95 disabled:opacity-30 transition-all uppercase tracking-widest" disabled>BAYAR SEKARANG</button>
                </div>
            </div>
        </div>
    </div>

    <div id="inventory-modal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-[250] p-4 backdrop-blur-sm no-print">
        <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-cart-item-enter">
            <div class="bg-orange-500 p-6 text-white flex justify-between items-center">
                ...
                <button onclick="document.getElementById('inventory-modal').classList.replace('flex','hidden')" class="text-4xl font-bold hover:text-black">&times;</button>
            </div>
            <div class="p-6">
                <div class="relative mb-6">
                    <input type="text" id="search-inventory" placeholder="Cari bahan kopi..." class="w-full p-4 pl-12 bg-gray-50 border border-gray-100 rounded-2xl outline-none text-sm font-bold focus:ring-2 focus:ring-orange-500 transition-all shadow-sm">
                ...
                <div id="inventory-list" class="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scroll no-scrollbar">
                    ...
                    <div class="inventory-item p-4 bg-gray-50 rounded-2xl transition-all border border-transparent hover:border-orange-100" data-name="{{ strtolower($bahan->nama_bahan) }}">
                        ...
                        @if(Auth::user()->role == 'admin')
                        <div class="flex space-x-2"><input type="number" id="restock-qty-{{ $bahan->id }}" placeholder="+ stok" class="flex-1 p-2 bg-white border border-gray-100 rounded-lg outline-none text-[10px] font-bold"><button onclick="restockItem({{ $bahan->id }})" class="bg-gray-900 text-white px-3 py-2 rounded-lg text-[9px] font-black uppercase hover:bg-orange-600 transition-all">Update</button></div>
                        @endif
                    </div>
                    @endforeach
                </div>
            </div>
            <div class="p-6 pt-0"><button onclick="document.getElementById('inventory-modal').classList.replace('flex','hidden')" class="w-full bg-gray-900 text-white py-4 rounded-2xl font-black uppercase text-xs hover:bg-black transition-all">Selesai Cek</button></div>
        </div>
    </div>

    <div id="receipt-modal" class="fixed inset-0 bg-black/60 hidden flex items-center justify-center z-[300] p-4 backdrop-blur-sm">
        <div id="modal-content" class="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl overflow-hidden transform scale-95 opacity-0 transition-all duration-300 text-center text-xs font-mono">
            ...
            <div class="p-8 pt-0"><button onclick="window.location.reload()" class="w-full bg-gray-900 text-white py-5 rounded-3xl font-black hover:bg-black transition-all uppercase tracking-widest shadow-xl">Selesai & Tutup</button></div>
        </div>
    </div>
    ...
    <script>
        ...
        // 1. FORCED LOGIN MODAL (WAJIB ISI!)
        ...
        function updateUIWithActiveShift() {
            ...
            if(name) {
                document.getElementById('main-greeting').innerHTML = `Halo, ${name}! ✨`;
        ...
        function startShift() {
            ...
            if(!name || !amount) return alert('⚠️ Tiara, wajib isi Nama Kasir & Modal Awal dulu ya!');
        ...
        function endShift() {
            ...
            localStorage.setItem('last_audit_html', res); localStorage.removeItem('shift_active'); localStorage.removeItem('current_cashier'); window.location.reload(); 
        }

        function voidTransaction(id) { if(confirm('Hapus transaksi? Stok bahan akan kembali.')) fetch(`/transaksi/${id}`, { method: "DELETE", headers: { "X-CSRF-TOKEN": "{{ csrf_token() }}" } }).then(() => window.location.reload()); }
        ...
        function performCheckout() {
            ...
            if(!n) return alert('⚠️ Isi Nama Pelanggan!'); 
