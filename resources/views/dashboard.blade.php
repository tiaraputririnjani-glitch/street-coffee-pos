...
<head>
    ...
    <title>Street Coffee POS - Master Final</title>
...
<body class="bg-gray-100 font-sans text-gray-900">
    ...
    <div id="shift-modal" class="fixed inset-0 bg-black/95 hidden items-center justify-center z-[500] p-4 backdrop-blur-2xl no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl border-4 border-orange-500">
            ...
            <p class="text-[10px] text-gray-400 font-bold mb-10 italic uppercase tracking-widest">Wajib Isi Sebelum Operasional</p>
    ...
    <div id="close-shift-modal" class="fixed inset-0 bg-black/80 hidden items-center justify-center z-[400] p-4 backdrop-blur-md no-print">
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl relative border-4 border-gray-900">
            ...
            <p class="text-xs text-gray-400 font-bold mb-8 italic text-red-500 uppercase">Input Uang Fisik Sekarang!</p>
                        ...
                        <table class="w-full text-left min-w-[600px]">
                            <thead>
                                <tr class="text-gray-400 text-[10px] uppercase font-black border-b border-gray-50">
                                    <th class="pb-4 px-2">Waktu</th><th class="pb-4 px-2">Nama Pelanggan</th><th class="pb-4 px-2">Detail Item</th><th class="pb-4 px-2">Metode</th><th class="pb-4 px-2 text-right">Total</th><th class="pb-4 text-center">Void</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50 text-xs font-bold text-gray-600">
                                ...
                                <tr class="group hover:bg-orange-50 transition-colors">
                                    ...
                                    <td class="py-5 px-2 text-[10px] font-black uppercase italic">{{ $trx->metode_pembayaran }}</td>
            ...
            <div id="section-rekap" class="hidden space-y-6 print-area">
                ...
                <div id="audit-result-area" class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button onclick="localStorage.removeItem('shift_active'); window.location.reload();" class="text-[9px] font-black text-red-400 uppercase border border-red-50 px-3 py-1 rounded-lg no-print">⚠️ Reset Sesi</button>
                    </div>
    ...
    <div id="inventory-modal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-[450] p-4 backdrop-blur-sm no-print">
    ...
    <script>
        ...
        // 1. FORCED SHIFT SYSTEM (MENGUNCI UNTUK KASIR)
        window.addEventListener('DOMContentLoaded', () => {
            const shiftActive = localStorage.getItem('shift_active');
            if (isOwner) {
                // Owner tidak dipaksa isi modal, tapi shift tetap dicatat jika perlu
        ...
        function updateUIWithActiveShift() {
            ...
            // Jika admin, abaikan nama shift di sapaan utama agar tetap "HAI OWNER!"
            if(isOwner) {
                greeting.innerHTML = "HAI OWNER! ☕";
                document.getElementById('active-cashier-label').innerText = name ? "Shift Berjalan: " + name : "Belum Ada Kasir";
            } else if(name) {
        ...
        function startShift() {
            ...
            if(!name || !amount) return alert('⚠️ Wajib isi Nama & Modal!');
        ...
        function updateCartUI() {
            ...
            if(cart.length === 0) { container.innerHTML = `<p class="py-10 text-center font-black opacity-20 uppercase text-[10px]">Pilih Menu...</p>`; totalDisplay.innerText = "Rp 0"; btnPay.disabled = true; return; }
